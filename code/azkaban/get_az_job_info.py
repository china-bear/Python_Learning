...
class SpyderAz(object):
    ...
    def fetch_jobs_from_flow(self, project, flow):
        ...
        response = fetch_jobs_from_flow_request(
            self.__session,
            self.__host,
            self.__session_id,
            project,
...
if __name__ == '__main__':
    ...
    az_user_dict = [{'url': 'http://ba1xx8083', 'user': 'xx', 'passwd': 'xx'}
                    ]
    sp = SpyderAz()
        ...
        for project in sp.fetch_projects(user):
            ...
            for flow in fetch_flows['flows']:
                ...
                try:
                    ...
                    schedule = 1
                    jobs = sp.fetch_jobs_from_flow(project, flow_name)
                    for job in jobs['nodes']:

                        job_name = job['id']

                        rs = f'"{url}","{user}","{project}","{flow_name}", "{job_name}", "{schedule}"'
                        print(rs)
